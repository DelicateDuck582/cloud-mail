...
- **🔐 附件签名防伪造**：COS/S3 私有桶存储，附件访问使用后端签发的短期 HMAC 签名（默认 15 分钟有效）+ Referer/Sec-Fetch 双层校验，防止盗链与伪造访问；直读代理 Worker（cos-exchange）验签通过后用 Cache API 按文件缓存（附件 key 为内容哈希，文件不变即无需回源，同一文件全局只回源一次）
- **📁 附件管理器**：独立的附件管理页面
