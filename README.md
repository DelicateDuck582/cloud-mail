...
- **🔐 附件签名防伪造**：COS/S3 私有桶存储，附件访问使用后端签发的短期 HMAC 签名（默认 15 分钟有效）+ Referer/Sec-Fetch 双层校验，防止盗链与伪造访问；并通过 Cloudflare 边缘缓存降低回源流量

- **📁 附件管理器**：独立的附件管理页面
  - 「全部 / 垃圾桶」双选项卡
  - 类型自动识别显示（`附件-图片` / `附件-Word文档` / `附件-PDF` / `附件-压缩包` / `附件-oc文件` 等，按扩展名）
  - 选中即操作：预览 / 下载 / 定位邮件 / 删除 / 恢复
  - 管理员可按用户筛选，显示所属用户与权限组
  - 点击文件名可直接预览（移动端友好）
  - 响应式布局，表格在容器内滚动不溢出

- **🗑️ 垃圾桶机制**：删除 = 移入垃圾桶（软删除，原文件不受影响），7 天后系统自动彻底清理；用户可以恢复自己的附件；**仅超级管理员可彻底删除**
