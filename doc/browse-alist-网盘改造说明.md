...
- **筛选/搜索（2026-09-25 修正）**：点「图片/视频/音频/…」选项卡或输入搜索词时，会**顺序扫描当前目录的全部页**
  （COS 不支持按类型过滤，旧实现只过滤当前页 → 文件多时点「视频」几乎必然空列表），命中项用本地数字分页展示，
  提示条写明「筛选「视频」：命中 12 个 · 已扫描 3 页（186 个文件）」；若**整个目录**都没有这类文件，提示
  「这个目录下没有「视频」文件，返回「全部」查看」并给出「返回「全部」」按钮（同时清空搜索词）。
  搜索输入 350 ms 防抖；扫描上限 30 页（超出会提示可能不全）；服务端 `/browse/api/list` 限速同步放宽到 120 次/分。
...
| 在线查看 | 点文件名/眼睛按钮打开预览：图片 `<img>`、文本按 Range 只取前 256 KB 显示在 `<pre>`、PDF `<iframe>`、音频/视频 `<video|audio controls preload="none">`（**不自动播放，须点击**）；`.svg` 按安全策略只提示下载；关闭（按钮/遮罩/Esc）时释放媒体并中断请求。服务端 `/temp/api/file` 支持单段 `Range`（206/416 + `Accept-Ranges`），可拖动进度。**类型判定按「扩展名 + MIME」双依据**（列表 `type` 是 MIME，如 `image/png`）；服务端对存储类型为 `octet-stream` 的文件按扩展名兜底为可内联类型（图片/音视频/文本），但**绝不映射为 html/svg 等可执行类型**（`.html/.svg` 只作文本查看，`text/html` 一律强制下载） |
