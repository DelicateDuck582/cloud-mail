...
| 在线查看 | 点文件名/眼睛按钮打开预览：图片 `<img>`、文本按 Range 只取前 256 KB 显示在 `<pre>`、PDF `<iframe>`、音频/视频 `<video|audio controls preload="none">`（**不自动播放，须点击**）；`.svg` 按安全策略只提示下载；关闭（按钮/遮罩/Esc）时释放媒体并中断请求。服务端 `/temp/api/file` 支持单段 `Range`（206/416 + `Accept-Ranges`），可拖动进度 |
